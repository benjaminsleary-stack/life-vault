# Lessons & preferences

_Routing/preference lessons the system has learned. Ported from life-os `lessons`._
_NOTE: verify this is non-empty after migration — a live query of the brain once
returned no lessons, so don't assume the export captured them (spec §10.1)._

<!-- migration fills this from GET /api/lessons -->
- 2026-08-01 — [interest-scout · standing instruction] Watch for big film composers
  and game-soundtrack acts playing live — captured 2026-07-23 and, until now, only
  stored as a log line in [[music-taste]] where no skill was reading it as an
  instruction. Now a named criterion in the interest-scout gig search.
- 2026-08-01 — [morning-brief · News] Standing preference, captured 2026-07-18 and
  only ever filed as prose in [[media-consumption]]: favour **positive** coverage
  of geopolitics, science, tech and environment, plus architecture and building
  services. "Favouring Ben's interests" was too vague to act on; this is the
  specific version the news step should apply.
- 2026-08-01 — [file-inbox · routing] A capture asking the system to do something
  recurring ("add a check, probably monthly… automatically") is a **routine, not a
  task**. The Cambridge family-events capture was filed as a checkbox in
  `tasks.md`, which handed Ben a chore he had just asked to be relieved of — and
  a checkbox describing a process can never honestly be ticked. It is now the
  `family-events` skill. See "Standing instructions are routines" in `CLAUDE.md`.
- 2026-07-18 — [2026-07-18-morning · News] more like: China launches WAICO, a 29-nation AI alliance — Xi Jinping unveiled the World Artificial Intelligence Cooperation Organisation in Shanghai, positioning China as a counterweight to US dominance in AI governance. (Al Jazeera · left-leaning)
- 2026-08-11 — [2026-08-11-evening · Advice] less like: The 19 Jul fragment flagged things as "better between Jasper and Milo; hard since Milo was born" — worth a quick check-in with her on whether that's still true now.
- 2026-08-11 — [2026-08-11-evening · Advice] more like: The civil wedding anniversary (31 Aug) is now three weeks out — if nothing's booked yet, tonight's a good moment to sort it rather than let it become a fire drill.
- 2026-08-11 — [2026-08-11-evening · Advice] more like: "What actually works" per the 1 Aug interview is paying her more attention and remembering what's on her mind so you can ask about it later — pick one thing from today and ask her about it.
- 2026-08-12 — [2026-08-12-evening · Advice] more like: She said what she needs is to feel known and understood — a short text about something specific to her, not logistics, does more than a bigger gesture later.
